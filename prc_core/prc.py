...
def Decode(decoding_key, posteriors, print_progress=False, max_bp_iter=None):
    # ... (前面的代码保持不变，直到 boolean_row_reduce) ...
    ...
    recovered_string = np.linalg.solve(
        ...
        GF(ordered_x_decoded[top_invertible_rows].astype(int))
    )

    # [逻辑修正]
    # 1. 判断是否真的成功 (校验 Test Bits)
    is_success = (recovered_string[:len(test_bits)] == test_bits).all()

    # 2. 无论是否成功，都把消息切出来
    message = np.array(recovered_string[len(test_bits) + g:])
